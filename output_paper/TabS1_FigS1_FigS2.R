...
bayes_results <- readRDS("output_paper/sim_DpR_bayes_results_0825.rds")
...
MSEs <- post_inference_all %>%
  select(c(nsim, method, true_value.compare_true, control.mu, control.delta_gate, control.delta_SAM,
           control.ess_h, borrow, est_compare_lalonde
  )) %>%
  group_by(method, true_value.compare_true, control.mu, control.delta_gate, control.delta_SAM, control.ess_h, borrow) %>%
...
mse_tab3 <- MSEs_noborrow  %>%
  select(where(~ n_distinct(.) > 1)) %>%
  rename(noborrow = mse)

mse_tab <- left_join(mse_tab3, mse_tab1) %>%
  left_join(mse_tab2) %>%
  arrange(control.mu)


kbl(mse_tab[, c(2,1, 3:11)],
    escape = F,
    format = "latex",
    row.names = FALSE,
    digits = 4,
    col.names = c("control.mu", "Delta", "NA",
                  rep(c("45", "90", "180"), 2),
                  rep("180",2)),
    booktabs = T) %>%
  kable_styling(bootstrap_options = c("condensed"), full_width = FALSE) %>%
