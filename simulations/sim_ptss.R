...
param_grid_h <- expand.grid(
  ctrl_h_n = 200, # Sample size for the historical control group.
  ctrl_h_prob = 0, # Probability of zero g-scores in the historical control group.
  ctrl_h_mu = 0.003, # Mean for the non-zero g-scores in the historical control group.
  ctrl_h_sigma = 1, # Standard deviation for the non-zero g-scores in the historical control group.
  stringsAsFactors = FALSE
)
...
param_grid1 <- expand.grid(
  trt_n = seq(15, 60, 5), # Sample size for the treatment group.
  trt_prob = 0, # Probability of zero g-scores in the treatment group.
  ctrl_prob = 0, # Probability of zero g-scores in the control group.
  ctrl_mu = c(0.003 * c(0.6, 0.8), 0.003, 0.003 / c(0.6, 0.8)),
  trt_sigma = 1, # Standard deviation for the non-zero g-scores in the treatment group.
  ctrl_sigma = 1, # Standard deviation for the non-zero g-scores in the control group.
  stringsAsFactors = FALSE
) %>%
...
saveRDS(all_historical_est, "all_historical_est_nozero.rds")
all_historical_est <- readRDS("all_historical_est_nozero.rds")
...
saveRDS(bayes_results, file = "sim3_gscore_bayes_results_nozero.rds")
saveRDS(metrics_approx_dist, file = "sim3_gscore_metrics_approx_dist_nozero.rds")

bayes_results <- readRDS("sim3_gscore_bayes_results_nozero.rds")
metrics_approx_dist <- readRDS("sim3_gscore_metrics_approx_dist_nozero.rds")
...
saveRDS(bayes_results_all, "sim3_gscore_bayes_results_df_nozero.rds")
...
results_all <- readRDS("sim3_gscore_bayes_results_df_nozero.rds")
...
metrics_df_long <- metrics_df_long[, c("control.median.ratio", "true_value.compare_true", "control.n",
                                       "bias_avg_median_ratio1_Frequentist", "sd_avg_Frequentist",
                                       "sd_empirical_Frequentist", "cp_Frequentist",
                                       "bias_avg_median_ratio1_Borrowing: No", "sd_avg_Borrowing: No",
                                       "sd_empirical_Borrowing: No", "cp_Borrowing: No",
...
metrics_df_long <- metrics_df_long[, c("control.median.ratio", "control.n", "true_value.compare_true",
                                       "bias_avg_median_ratio1_Borrowing: No", "sd_avg_Borrowing: No",
                                       "sd_empirical_Borrowing: No", "cp_Borrowing: No",
                                       "bias_avg_median_ratio1_Borrowing: Yes", "sd_avg_Borrowing: Yes",
                                       "sd_empirical_Borrowing: Yes", "cp_Borrowing: Yes")]
...
kbl(metrics_df_long[, -1], escape = F, row.names = F, digits = 3,
    format    = "latex",
    booktabs  = T,
    col.names = c("Sample size","treatment effect",
                  "BIAS", "SD-avg","SD-emp", "CP",
                  "BIAS", "SD-avg","SD-emp", "CP")) %>%
  kable_styling(bootstrap_options = c("condensed")) %>%
  kable_classic(full_width = FALSE) %>%
...
metrics_df$control.median.ratio <- factor(as.character(round(metrics_df$control.median.ratio, 2)),
                                          levels = c("0.6", "0.8", "1", "1.25", "1.67"),
                                      labels=c(expression(paste(theta[c], " / ", theta[ch], " = 0.60")),
                                               expression(paste(theta[c], " / ", theta[ch], " = 0.80")),
                                               expression(paste(theta[c], " / ", theta[ch], " = 1.00")),
                                               expression(paste(theta[c], " / ", theta[ch], " = 1.25")),
                                               expression(paste(theta[c], " / ", theta[ch], " = 1.67"))))

p1 <- ggplot(metrics_df, aes(x = control.n, y = bias_avg_median_ratio1, color = Approach)) +
  geom_point(position = position_dodge(width = 0.3), size = 2) +
  facet_grid(true_value.compare_true~control.median.ratio, labeller = labeller(control.median.ratio = label_parsed)) +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  labs(x = "Sample size per arm", y = "BIAS", color= "") +
  theme_bw() +
  theme(legend.position = "bottom")
...
oc_all <- oc_all[, c("i",
                     ...
                     "true_value.control.true_value",
                     "control.n", "borrowing", "control_h.prob",
...
ggsave("sim3_gscore_conflict_vs_risk_nozero.jpg", width = 16, height = 4)
...
p_oc <- p_list[[1]] + p_list[[2]] +
  ...
  theme(legend.position = "bottom",
        ...
        axis.text = element_text(size = 12),
        strip.text = element_text(size = 13))
ggsave("sim3_gscore_zone_size_nozero.jpg",p_oc, width = 18, height = 8)
