...
for (i in seq_along(data_gen_params_list)) {
  ...
  # prior_grid <- expand.grid(
  #   treatment.theta0 = as.numeric(summary_h["control_h.mu_hat"]),
  #   treatment.s0 = as.numeric(data_gen_params_h$control_h$sigma) * sqrt(as.numeric(data_gen_params_h$control_h$n)),
  #   treatment.w = 0,
  #   control.theta0 = as.numeric(summary_h["control_h.mu_hat"]),
  #   control.s0 = as.numeric(data_gen_params_h$control_h$sigma) * sqrt(as.numeric(data_gen_params_h$control_h$n)),
  #   control.w = c(NA, 0),
  #   control.delta_gate = c(0.1, 0.15),
  #   control.ess_h = c(45, 90, 180),
  #   stringsAsFactors = FALSE
  ...
  for (k in seq_along(prior_params_list)) {
    ...
    end_time_k <- Sys.time()
    cat("Time for Bayesian analysis =", round(difftime(end_time_k, start_time_k, units = "secs"), 2), "seconds\n\n")
  }
...
saveRDS(bayes_results, file = "simulations/sim_DpR_unconstrained_bayes_results_5_18.rds")
...
saveRDS(bayes_results_all, "simulations/sim_DpR_unconstrained_bayes_results_df_5_18.rds")


# Analysis -------------------------------------------------------------

bayes_results_all <- readRDS("simulations/sim_DpR_unconstrained_bayes_results_df_5_18.rds")
...
shade_df <- data.frame(
  control.delta_gate = factor(c(0.1, 0.15), levels = c("0.1", "0.15"),
                              labels = c(expression(paste(delta, " = 0.1")),
                                         expression(paste(delta, " = 0.15")))),
  xmin = c(2, 1),
  xmax = c(6, 7),
...
library(patchwork)
p1 / p2
ggsave(filename = "simulations/sim_DpR_unconstrained_SAM_weight_5_18.jpg", p1 / p2,
       width = 10, height = 8)
...
metrics_tab <- post_inference_all %>%
  group_by(control.delta_gate, control.ess_h, control.mu) %>%
...
risk_df <- bayes_results_all$oc_all %>%
  ...
  mutate(
    CorrectLabel = case_when(
      true_value.compare_true == -0.2 & decision_pr == "go" ~ "CorrectGo",
      true_value.compare_true == -0.1 & decision_pr == "no-go" ~ "CorrectStop",
      TRUE ~ NA_character_
    ),
    RiskLabel = case_when(
  ...
  filter(!is.na(label)) %>%
  group_by(borrowing, control.delta_gate, control.ess_h, control.mu, label) %>%
...
metrics_tab <- merge(metrics_tab, risk_df,
                     by = c("control.delta_gate", "control.ess_h","control.mu"),
                     all = TRUE)

metrics_tab_1 <- metrics_tab %>%
  mutate(mean_pmd = ifelse(borrowing == "No", NA, mean_pmd),
         sd_pmd = ifelse(borrowing == "No", NA, sd_pmd)) %>%
  pivot_longer(c(FGR, FSR, CorrectGo, CorrectStop, mean_pmd, sd_pmd)) %>%
  pivot_wider(names_from = c(borrowing, control.delta_gate, control.ess_h),
              values_from = value, names_sort = T) %>%
  mutate(name = factor(name, levels = c("FGR", "FSR", "CorrectGo", "CorrectStop", "mean_pmd", "sd_pmd"))) %>%
  select(c(2,1,3,9:14)) %>%
  arrange(name, control.mu) %>%
  # mutate(metric = paste0(name, "\n", true_value.compare_true)) %>%
  # mutate(metric = kableExtra::linebreak(metric, align = 'c')) %>%
  select(c(1,2, 4:9, 3))
...
no_borrowing_df <- risk_df %>%
  filter(borrowing == "No") %>%
  mutate(control.delta_gate = NA) %>%
  unique()
...
p_risk <- ggplot(risk_df %>% filter(borrowing == "Yes"),
                 aes(x = control.mu.diff, y = value, color = name)) +
  geom_point(size = 2) +
  geom_rect(
    data = shade_df,
    aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax,
...
ggsave( "simulations/sim_DpR_unconstrained_conflict_vs_risk_5_18.jpg", p_risk, width = 10, height = 5)
...
oc_new <- oc_new[, c("true_value.compare_true", "control.delta_gate", "control.ess_h",
                     "borrowing",
                     "control.mu.diff",
...
oc_new$borrowing <- factor(oc_new$borrowing,
                           levels = c("No", "Yes"),
                           labels = c("Borrowing: No", "Borrowing: Yes"))
oc_new$control.delta_gate <- factor(oc_new$control.delta_gate,
                                    levels = c("0.1", "0.15"),
                                    labels = c(expression(paste(delta, " = 0.1")),
                                               expression(paste(delta, " = 0.15"))))
...
for (i in levels(oc_new$true_value.compare_true)) {
  ...
  p_list[[i]] <- ggplot(oc_tmp,
                        aes(x = as.factor(control.ess_h), y = proportion_pr, fill = decision_pr)) +
    ...
    facet_grid(control.mu.diff~control.delta_gate+borrowing,
               labeller = labeller(control.mu.diff = label_parsed,
                                   control.delta_gate = label_parsed,
...
ggsave("simulations/sim_DpR_unconstrained_zone_size_5_18.jpg", p_oc, width = 14, height = 14)
