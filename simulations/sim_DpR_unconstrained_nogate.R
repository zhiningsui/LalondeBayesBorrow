...
for (i in seq_along(data_gen_params_list)) {
  ...
  # Define Priors (w/w.o. gated control, w/w.o borrowing)
...
saveRDS(bayes_results, file = "simulations/sim_DpR_unconstrained_bayes_results_5_31.rds")
...
saveRDS(bayes_results_all, "simulations/sim_DpR_unconstrained_bayes_results_df_5_31.rds")
...
saveRDS(bayes_results, file = "simulations/sim_DpR_unconstrained_bayes_results_5_23_np.rds")
...
saveRDS(bayes_results_all, "simulations/sim_DpR_unconstrained_bayes_results_df_5_23_np.rds")
...
saveRDS(bayes_results, file = "simulations/sim_DpR_unconstrained_bayes_results_5_23_np_sigma40.rds")
...
saveRDS(bayes_results_all, "simulations/sim_DpR_unconstrained_bayes_results_df_5_23_np_sigma40.rds")



# Analysis I -------------------------------------------------------------

bayes_results_all <- readRDS("simulations/sim_DpR_unconstrained_bayes_results_df_5_23.rds")
...
post_params_all <- bayes_results_all$post_params_all %>%
  ...
  mutate(control.mu.diff = factor(control.mu.diff,
         ...
         control.delta_SAM = factor(control.delta_SAM,
                                     levels = c("0.05", "0.1", "0.15"),
                                     labels = c(expression(paste(delta, " = 0.05")),
...
shade_df <- data.frame(
  control.delta_SAM = factor(c(0.05, 0.1, 0.15),
                             levels = c("0.05", "0.1", "0.15"),
                              labels = c(expression(paste(delta, " = 0.05")),
                                         expression(paste(delta, " = 0.1")),
                                         expression(paste(delta, " = 0.15")))),
...
library(patchwork)
p1 / p2
ggsave(filename = "simulations/sim_DpR_unconstrained_SAM_weight_5_23.jpg", p1 / p2,
       width = 16, height = 10)
...
ggsave( "simulations/sim_DpR_unconstrained_conflict_vs_risk_5_23.jpg", p_risk, width = 11, height = 6)
...
p_oc <- p_list[[1]] + p_list[[2]] +
  ...
  theme(legend.position = "bottom",
        ...
        strip.text = element_text(size = 12))
p_oc
ggsave("simulations/sim_DpR_unconstrained_zone_size_5_23.jpg", p_oc, width = 20, height = 15)


# Borrowing only
oc_new_borrow <- oc_new[oc_new$borrowing == "Borrowing: Yes",]
p_list <- list()
for (i in levels(oc_new_borrow$true_value.compare_true)) {
  oc_tmp <- oc_new_borrow[oc_new_borrow$true_value.compare_true == i, ]
  ...
  p_list[[i]] <- ggplot(oc_tmp,
    ...
    geom_text(aes(y = label_ypos, label = scales::percent(proportion_pr, 0.01)),
              ...
              size = 2) +
    scale_fill_manual(name = "Decision", values = myColors)+
    facet_grid(control.ess_h+gate~control.delta_SAM,
               labeller = labeller(control.ess_h = label_parsed,
                                   control.delta_SAM = label_parsed)) +
    ggtitle(bquote(Delta == .(i))) +
    labs(x =expression(paste(Delta[c], " = ", theta[c], " - ", theta[ch])), y = "Probability") +
...
p_oc <- p_list[[1]] + p_list[[2]] +
  ...
  theme(legend.position = "bottom",
        ...
        strip.text = element_text(size = 12))
p_oc
ggsave("simulations/sim_DpR_unconstrained_zone_size_5_23_borrow.jpg", p_oc, width = 20, height = 12)
...
bayes_results_all <- readRDS("simulations/sim_DpR_unconstrained_bayes_results_df_5_23_np.rds")
...
library(patchwork)
p1 / p2
ggsave(filename = "simulations/sim_DpR_unconstrained_SAM_weight_5_23_np.jpg", p1 / p2,
       width = 13, height = 8)
...
ggsave( "simulations/sim_DpR_unconstrained_conflict_vs_risk_5_23_np.jpg", p_risk, width = 10, height = 5)
...
p_oc <- p_list[[1]] + p_list[[2]] +
  ...
  theme(legend.position = "bottom",
        ...
        strip.text = element_text(size = 12))
p_oc
ggsave("simulations/sim_DpR_unconstrained_zone_size_5_23_np.jpg", p_oc, width = 20, height = 15)
...
bayes_results_all <- readRDS("simulations/sim_DpR_unconstrained_bayes_results_df_5_23_np_sigma40.rds")
...
library(patchwork)
p1 / p2
ggsave(filename = "simulations/sim_DpR_unconstrained_SAM_weight_5_23_np_sigma40.jpg", p1 / p2,
       width = 13, height = 8)
...
ggsave( "simulations/sim_DpR_unconstrained_conflict_vs_risk_5_23_np_sigma40.jpg", p_risk, width = 10, height = 5)
...
p_oc <- p_list[[1]] + p_list[[2]] +
  ...
  theme(legend.position = "bottom",
        ...
        strip.text = element_text(size = 12))
p_oc
ggsave("simulations/sim_DpR_unconstrained_zone_size_5_23_np_sigma40.jpg", p_oc, width = 20, height = 15)
