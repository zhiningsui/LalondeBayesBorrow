...
posterior_distribution <- function(endpoint,
                                   current,
                                   historical = NULL,
                                   delta_SAM = NULL,
                                   delta_gate = NULL,
  ...
  if (!is.character(endpoint) ||
      length(endpoint) != 1 || !(endpoint %in% supported_endpoints)) {
    stop(
      paste(
        "Input 'endpoint' must be a single string, either 'continuous' or 'binary'. Supported endpoints are:",
        paste(supported_endpoints, collapse = ", ")
      )
    )
  }
  ...
  if (!is.null(historical) &&
      !is.list(historical) && !is.vector(historical)) {
  ...
  if (!all(hasName(current, req_current_names))) {
    stop(
      paste(
        "Current data is missing required elements for",
        endpoint,
        "endpoint:",
        paste(setdiff(req_current_names, names(current)), collapse = ", ")
      )
    )
  }
  if (!is.null(historical) &&
      !all(hasName(historical, req_historical_names))) {
    stop(
      paste(
        "Historical data is provided but is missing required elements for",
        endpoint,
        "endpoint:",
        paste(setdiff(
          req_historical_names, names(historical)
        ), collapse = ", ")
      )
    )
  }
  ...
  extract_and_validate <- function(data,
                                   name,
                                   param_name,
                                   is_positive = FALSE,
    ...
    if (!is.numeric(val) || length(val) != 1 || !is.finite(val)) {
      stop(
        paste(
          ...
          "' in ",
          name,
          " data must be a single finite numeric value.",
          sep = ""
        )
      )
    }
  ...
  n <- extract_and_validate(current,
                            "current",
                            "n",
                            is_positive = TRUE,
                            is_integer = TRUE)
  ...
  if (endpoint == "continuous") {
    ...
    # binary
    x <- extract_and_validate(current,
                              "current",
                              "count",
  ...
  extract_prior_param <- function(param,
                                  name,
                                  is_positive = FALSE,
                                  is_non_negative = FALSE) {
    if (is.null(param)) {
    ...
    if (!is.numeric(param) ||
        length(param) != 1 || !is.finite(param)) {
      stop(paste(
        "Prior parameter '",
        name,
  ...
  if (is.null(ess_h)) {
    ...
    if (!is.numeric(ess_h) ||
        length(ess_h) != 1 || !is.finite(ess_h) || ess_h < 0) {
      stop(
        "Parameter 'ess_h' must be a single non-negative finite numeric value when not NULL."
      )
    }
    ess_h_used <- ess_h
  }
  ...
  if (ess_h_used < .Machine$double.eps) {
    # Treat near-zero ess_h as zero
    ess_h_used <- 0
    if (!is.null(w) && w > 0) {
    ...
    if (!is.null(w)) {
      if (!is.numeric(w) ||
          length(w) != 1 || !is.finite(w) || w < 0 || w > 1) {
  ...
  if (is.null(w_input_used) && power_alpha > 0) {
    if (is.null(delta_SAM) ||
        !is.numeric(delta_SAM) ||
        length(delta_SAM) != 1 ||
        !is.finite(delta_SAM) || delta_SAM <= 0) {
      delta_SAM <- if (endpoint == "continuous")
        0.2
      else
        0.1
      warning(
        paste(
          "Parameter 'delta_SAM' was NULL or invalid. Using default delta_SAM =",
          delta_SAM,
          "for",
          endpoint,
          "endpoint."
    ...
    if (endpoint == "continuous") {
      if (is.na(sh) ||
          sh <= 0)
        stop(
          "Historical standard deviation 's' must be positive for SAM calculation with continuous endpoint."
        )

      # Pooled standard deviation
      pooled_n_total <- nh + n
      df_pooled <- if (pooled_n_total > 2)
        pooled_n_total - 2
      else
        1 # Avoid negative or zero df
      ...
      if (!is.null(delta_gate) && abs(y - yh) >= delta_gate) {
        ...
        if (s_pooled < .Machine$double.eps) {
          ...
          loglik_obs_diff <- dnorm(y - yh,
                                   mean = 0,
                                   sd = s_pooled,
                                   log = TRUE)

          # Maximum likelihood at shifted difference (delta or -delta)
          loglik_shifted_plus <- dnorm(y - yh,
                                       mean = delta_SAM,
                                       sd = s_pooled,
                                       log = TRUE)
          loglik_shifted_minus <- dnorm(y - yh,
                                        mean = -delta_SAM,
                                        sd = s_pooled,
                                        log = TRUE)
      ...
      if (!is.null(delta_gate) &&
          abs(thetac_current_mean - thetah_prior_mean) >= delta_gate) {
        # message("Gate applied: |current - historical| >= delta_gate. Setting w_prior = 0.")
        w_prior <- 0
      } else {
        # Otherwise, compute SAM prior weight as usual
        # if (!is.null(delta_gate)) {
        #   message("Gate not triggered: |current - historical| < delta_gate. Proceeding with SAM prior.")
        # }

        if (thetah_prior_mean <= .Machine$double.eps ||
            thetah_prior_mean >= 1 - .Machine$double.eps) {
          warning("Historical prior mean near 0/1. Unstable SAM weight. Forcing w_prior = 0.")
          w_prior <- 0
        } else {
          loglik_obs <- dbinom(x,
                               size = n,
                               prob = thetah_prior_mean,
                               log = TRUE)
          # Calculate likelihood of current data under shifted probabilities (+/- delta)
          thetah_plus_delta <- min(thetah_prior_mean + delta_SAM,
                                   1 - .Machine$double.eps) # Subtract tiny epsilon to avoid prob=1 edge case in dbinom
          ...
          loglik_shifted_plus <- dbinom(x,
                                        size = n,
                                        prob = thetah_plus_delta,
                                        log = TRUE)
          loglik_shifted_minus <- dbinom(x,
                                         size = n,
                                         prob = thetah_minus_delta,
                                         log = TRUE)
          loglik_max_shifted <- max(loglik_shifted_plus, loglik_shifted_minus)
  ...
  if (w_prior < .Machine$double.eps) {
    # Treat near-zero prior weight as zero
    w_prior <- 0
    ws <- 0
  ...
  } else if (w_prior > 1 - .Machine$double.eps) {
    # Treat near-one prior weight as one
    w_prior <- 1
    ws <- 1
    ...
    # Case 3: Borrowing occurs with prior weight > 0 and < 1 (either fixed or SAM)

    if (endpoint == "continuous") {
      if (is.na(sh) || sh <= 0) {
        stop(
          "Historical standard deviation 's' must be positive to calculate informative posterior component."
        )
      }
      ...
      log_part1_lik <- dnorm(
        y,
        mean = yh,
        sd = sqrt(sh_discount_var + s^2),
        log = TRUE
      )
      log_part2_lik <- dnorm(y,
                             mean = theta0,
                             sd = sqrt(s0^2 + s^2),
                             log = TRUE)
      ...
      if (is.na(xh) || is.na(nh) || nh <= 0) {
        stop(
          "Internal error: Historical data invalid for informative posterior calculation."
        )
      }
  ...
  if (post$w < .Machine$double.eps) {
    # Treat near-zero posterior weight as zero
    post$w <- 0
    if (endpoint == "continuous") {
      post$mu1 <- NA_real_
  ...
  } else if (post$w > 1 - .Machine$double.eps) {
    # Treat near-one posterior weight as one
    post$w <- 1
    if (endpoint == "continuous") {
      post$mu2 <- NA_real_
...
DPP.posterior = function(Yt=39, nt=60, Yc=13, nc=30,
                        ...
                        theta = 0.5, eta = 1){
  w = BayesianHybridDesign::borrow.wt(Yc = Yc, nc = nc, Ych = Ych,
  ...
  post_t$b = bpost_t
  post_c$w <- 1
  post_c$a = apost_c_hca
  post_c$b = bpost_c_hca

  return(list(post_c = list(w_prior = unname(w), post = post_c),
              post_t = list(w_prior = unname(w), post = post_t)))
}
